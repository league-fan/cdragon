...
const skinsResolvedMap = (skins: Skin[], skinInfos: SkinInfo[]) => {
  return skins.map((skin) => ({
    ...
    rarity: skin.rarity,
    isBase: skin.isBase,
    wikiPrice: skinInfos.find((info) => info.id === skin.id)?.price,
    wikiReleaseDate: skinInfos.find((info) => info.id === skin.id)?.releaseDate,
  }));
};
...
class Crawler {
  ...
  async crawling(skinInfos?: SkinInfo[]) {
    skinInfos = skinInfos || [];
...
async function main() {
  ...
  // 获取SkinInfo
  const defaultChampions = await defaultCrawler.api.fetchAsset(
    "v1/champion-summary.json",
  );
  const skinInfos =
    (await concurrentLimit(defaultChampions, async (champion) => {
      console.log(`start crawling ${champion.name} skins`);
      try {
        return await getChampionSkins(champion.name);
      } catch (error) {
        console.error(
          `crawl ${champion.name} skins error: ${JSON.stringify(error)}`,
        );
        return [];
      }
    }, 10)).flat();
  ...
  await Promise.all(desiredLanguages.map(async (language) => {
    ...
    await crawler.crawling(skinInfos);
